...
const triggerEvent$1 = function(eventName, { onElement, bubbles, cancelable, attributes } = {}) {
...
class AttachmentView extends ObjectView {
  ...
  createCaptionElement() {
    const figcaption = makeElement({ tagName: "figcaption", className: css$3.attachmentCaption });
    ...
    if (caption) {
      ...
      if (name) {
        const nameElement = makeElement({ tagName: "span", className: css$3.attachmentName, textContent: name });
      ...
      if (size) {
        ...
        const sizeElement = makeElement({ tagName: "span", className: css$3.attachmentSize, textContent: size });
  ...
  getClassName() {
    const names = [ css$3.attachment, `${css$3.attachment}--${this.attachment.getType()}` ];
...
const createCursorTarget$1 = (name) =>
  makeElement({
    tagName: "span",
...
const { lang, css: css$1, keyNames: keyNames$1 } = config;
...
class AttachmentEditorController extends BasicObject {
  ...
  addToolbar = undoable(() => {
    ...
    triggerEvent$1("trix-attachment-before-toolbar", { onElement: this.element, attributes: { toolbar: element, attachment: this.attachment } });
  ...
  applyKeyboardCommand(keys) {
    ...
    for (const button of Array.from(this.element.querySelectorAll("[data-trix-key]"))) {
      ...
      if (buttonKeyString === keyString) {
        triggerEvent$1("mousedown", { onElement: button });
        return true
      }
    }
    return false
...
class TrixEditorElement extends HTMLElement {
  ...
  notify(message, data) {
    if (this.editorController) {
      return triggerEvent$1(`trix-${message}`, { onElement: this, attributes: data })
    }
  }
  ...
  connectedCallback() {
    if (!this.hasAttribute("data-trix-internal")) {
      ...
      if (!this.editorController) {
        triggerEvent$1("trix-before-initialize", { onElement: this });
        this.editorController = new EditorController({
          editorElement: this,
          html: this.defaultValue = this.value,
        });
        requestAnimationFrame(() => triggerEvent$1("trix-initialize", { onElement: this }));
      }
...
const createEvent$1 = function (type, properties = {}) {
  const event = document.createEvent("Events");
...
const testIf = function (condition, ...args) {
  if (condition) {
...
const moveCursor = async (options) => {
  ...
  const move = async () => {
    ...
    if (triggerEvent(document.activeElement, "keydown", { keyCode: keyCodes$1[direction], key: keys[direction] })) {
...
const expandSelection = async (options) => {
  ...
  const expand = async () => {
    ...
    if (triggerEvent(document.activeElement, "keydown", { keyCode: keyCodes$1[direction], key: keys[direction], shiftKey: true })) {
...
const triggerInputEvent = function (element, type, properties = {}) {
  if (input.getLevel() === 2) {
    let ranges;
...
const createFile$1 = function (properties = {}) {
  const file = {
    getAsFile() {
      return {}
    },
...
const clickToolbarDialogButton = async ({ method }) => {
...
const typeInToolbarDialog = async (string, { attribute }) => {
...
testGroup("Attachment", () => {
  ...
  previewableTypes.forEach((contentType) => {
    test$3(`${contentType} content type is previewable`, () => {
      assert.ok(createAttachment({ contentType }).isPreviewable());
    });
  });

  nonPreviewableTypes.forEach((contentType) => {
    test$3(`${contentType} content type is NOT previewable`, () => {
...
const withPreviewCaptionConfig = (captionConfig, fn) => {
  ...
  const { caption } = attachments.preview;
